...
if sys.platform != "win32":
    ...
    # Placeholder on Windows so the module can be imported for doctest discovery
    class PosixPtyProcess:  # type: ignore[no-redef]
        pass

    def spawn(argv: list[str], rows: int, cols: int) -> PosixPtyProcess:  # type: ignore[misc]
        raise NotImplementedError("POSIX pty backend is not available on Windows")
