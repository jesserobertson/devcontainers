...
def merge_layer(base: dict[str, Any], overlay: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, overlay_value in overlay.items():
        if key in SCALAR_FIELDS:
            ...
            result[key] = merge_lifecycle_command(result.get(key), overlay_value)
        elif key in FEATURE_FIELDS:
            result[key] = merge_feature_map(result.get(key), overlay_value)
        elif key in ARRAY_CONCAT_FIELDS:
            result[key] = merge_array_concat(result.get(key), overlay_value)
        elif key in ARRAY_FIELDS:
            result[key] = merge_array_dedup(result.get(key), overlay_value)
        elif key in MAP_FIELDS:
            result[key] = merge_map(result.get(key), overlay_value)
        else:
            result[key] = overlay_value
    return result
...
def merge_lifecycle_command(base_value: Any, overlay_value: Any) -> Any:
...
def merge_feature_map(base_value: Any, overlay_value: Any) -> Any:
...
def merge_array_dedup(base_value: Any, overlay_value: Any) -> list[Any]:
...
def merge_array_concat(base_value: Any, overlay_value: Any) -> list[Any]:
    if not isinstance(overlay_value, list):
        return list(base_value) if isinstance(base_value, list) else []
    merged = list(base_value) if isinstance(base_value, list) else []
...
def merge_map(base_value: Any, overlay_value: Any) -> dict[str, Any]:
    if not isinstance(overlay_value, dict):
